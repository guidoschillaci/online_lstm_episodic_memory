...
if __name__ == "__main__":
    ...
    for exp in range(experiments):
        for iter in range(iterations):
            ...
            plt.ylim(0, 0.5)
            plt.title('Mean Squared Error - MemStrategy: '+ str(int(parameters['memory_update_strategy'])) + ' iteration '+ str(iter))
            plt.savefig(directory + 'learning_progress.png')
            plt.show()
            ...
            ax.axvline(x=switch_time[0],color='r', linestyle='dashed')
            ax.axvline(x=switch_time[1],color='b', linestyle='dashed')
            plt.title('Input variances - MemStrategy: ' + str(int(parameters['memory_update_strategy'])) + ' iteration '+ str(iter))
            ...
            ax.axvline(x=switch_time[0],color='r', linestyle='dashed')
            ax.axvline(x=switch_time[1],color='b', linestyle='dashed')
            plt.title('Output variances - MemStrategy: ' + str(int(parameters['memory_update_strategy'])) + ' iteration '+ str(iter))
            ...
            ax.axvline(x=switch_time[0],color='r', linestyle='dashed')
            ax.axvline(x=switch_time[1],color='b', linestyle='dashed')
            plt.title('Which greenhouse data is used - MemStrategy: ' + str(int(parameters['memory_update_strategy'])) + ' iteration '+ str(iter))
            ...
            plt.title('Memory Index - MemStrategy '  + str(int(parameters['memory_update_strategy'])) + ' iteration '+ str(iter))
